    ...
    void Vision::getAnglesBySig(int sig_id, Pronounce::Position* positions) {
        ...
        if (this->get_object_count() < 1) {
            ...
            for (int i = 0; i < index; i++) {
                double x = objectArr[i].x_middle_coord; // Pronounce::mapFunc(objectArr[i].x_middle_coord, -320, 320, -30.5, 30.5);
                double y = objectArr[i].y_middle_coord; // Pronounce::mapFunc(objectArr[i].y_middle_coord, -200, 200, -20.5, 20.5);
                positions[i].setX(x);
                positions[i].setY(y);
            }
