	...
	public:
		...
		double get(double key) {
			if (key < values.at(0).first) {
				double t = map(key, values.at(0).first, values.at(1).first, 0, 1);
				return lerp(values.at(0).second, values.at(1).second, t);
			}

			for (int i = 0; i < values.size() - 1; i++) {
				if (key > values.at(i).first) {
					double t = map(key, values.at(i).first, values.at(i+1).first, 0, 1);
					return lerp(values.at(i).second, values.at(i+1).second, t);
				}
			}

			double t = map(key, values.at(values.size()-2).first, values.at(values.size()-1).first, 0, 1);
