...
namespace Pronounce {
	...
	public:
		...
		void update() {
			...
			double power = controller->get_analog(E_CONTROLLER_ANALOG_LEFT_Y) / 127.0;
			double turn = controller->get_analog(E_CONTROLLER_ANALOG_RIGHT_X) / 127.0;

			// double power = (left + right)/2.0;
			// double turn = (left - right)/2.0;
